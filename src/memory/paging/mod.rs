...
pub fn remap_the_kernel<A>(allocator: &mut A, boot_info: &BootInformation)
  where A: FrameAllocator
{
  let mut temporary_page = TemporaryPage::new(Page { number: 0xcafedba}, allocator);
  ...
  active_table.with(&mut new_table, &mut temporary_page, |mapper| {
    ...
    let elf_sections_tag = boot_info.elf_sections_tag().expect("memory map tag");
    for section in elf_sections_tag.sections() {
      ...
      assert!(section.start_address() % PAGE_SIZE == 0, "not aligned");
